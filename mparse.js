  ...
  app.get("/parse", function(req, res) {
    ...
    function readDirectory() {
      return new Promise((resolve, reject) => {
        ...
        fs.readdir(`/tmp/stage`, (err, files) => {
          if (err) {
            reject(err);
          }
    ...
    function parseData(schemas, keyed_lookup, cluster_lookup, files) {
      ...
      const parsed_files = Promise.map(files, (file) => {
        ...
        return new Promise((resolve, reject) => {
          ...
          fs.readFile(`/tmp/stage/${file}`, 'utf8', (err, data) => {
            if (err) {
              return reject(err);
            }
        ...
        return new Promise((resolve, reject) => {
          request({ url, encoding: null }, function(err, resp, body) {
            ...
            fs.writeFile(`${outputDir}${fileName}`, body, function(err) {
              ...
              stream.pipe(unzip.Extract({ path: `/tmp/stage` })
                .on('close', function() {
    ...
    function readyWorkspace() {
      return new Promise((resolve, reject) => {
        ...
        const directories_in_order = ['/tmp/group', '/tmp/stage', '/tmp/output'];
